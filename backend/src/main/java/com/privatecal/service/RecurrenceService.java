...
public class RecurrenceService {
    ...
    public void addExceptionDate(Task task, LocalDateTime exceptionDateLocal) {
        // CRITICAL: We need to find the actual occurrence Instant generated by ical4j,
        // not just convert LocalDateTime naively to UTC.
        // This is because ical4j generates occurrences by adding intervals in UTC,
        // which means the LocalDateTime->UTC conversion changes across DST transitions.

        Instant exceptionInstant = findOccurrenceInstant(task, exceptionDateLocal);

        if (exceptionInstant == null) {
            // Fallback: if we can't find the occurrence, use naive conversion
            // This shouldn't happen for valid occurrences
            logger.warn("Could not find occurrence for {} in task {}, using naive conversion",
                       exceptionDateLocal, task.getUid());
            exceptionInstant = exceptionDateLocal
        ...
        logger.info("Added exception date {} (local) / {} (instant/UTC) to task {}",
                   exceptionDateLocal, exceptionInstant, task.getUid());
    }

    /**
     * Find the actual Instant of an occurrence given its LocalDateTime.
     * This searches through generated occurrences to find the one that matches
     * the LocalDateTime when converted to the task's timezone.
     *
     * This is necessary because ical4j generates occurrences in UTC by adding intervals,
     * so the LocalDateTime->UTC mapping changes across DST transitions.
     */
    private Instant findOccurrenceInstant(Task task, LocalDateTime targetLocal) {
        if (task.getRecurrenceRule() == null || task.getRecurrenceRule().trim().isEmpty()) {
            // Non-recurring task
            return null;
        }
        ...
        try {
            ...
            // Search in a reasonable range around the target date
            // (we need to generate occurrences to find the matching one)
            LocalDate targetDate = targetLocal.toLocalDate();
            Instant searchStart = targetDate.minusDays(1).atStartOfDay(taskZone).toInstant();
            Instant searchEnd = targetDate.plusDays(2).atStartOfDay(taskZone).toInstant();

            // Generate occurrences in this range
            List<TaskOccurrence> occurrences = expandRecurrences(task, searchStart, searchEnd);

            // Find the occurrence that matches the target LocalDateTime
            for (TaskOccurrence occurrence : occurrences) {
                LocalDateTime occurrenceLocal = occurrence.getOccurrenceStart()
                    .atZone(taskZone)
                    .toLocalDateTime();

                // Match by LocalDateTime (allowing 1 minute tolerance for rounding)
                if (Math.abs(java.time.Duration.between(occurrenceLocal, targetLocal).toMinutes()) <= 1) {
                    logger.debug("Found matching occurrence: {} (local) = {} (UTC)",
                                occurrenceLocal, occurrence.getOccurrenceStart());
                    return occurrence.getOccurrenceStart();
                }
            }

            logger.warn("No matching occurrence found for {} in task {}", targetLocal, task.getUid());
            return null;

        } catch (Exception e) {
            logger.error("Error finding occurrence instant for {} in task {}: {}",
                        targetLocal, task.getUid(), e.getMessage());
            return null;
        }
    }
